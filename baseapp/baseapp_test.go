...
func setupBaseApp(t *testing.T, options ...AppOption) *BaseApp {
	options = append(options, SetSubstores(capKey1, capKey2))
...
func TestLoadVersionPruning(t *testing.T) {
	...
	app = NewBaseApp(name, logger, db, nil, pruningOpt, schemaOpt)
...
func testChangeNameHelper(name string) AppOptionFunc {
	return func(bap *BaseApp) { bap.SetName(name) }
}
...
func TestInitChainer(t *testing.T) {
	...
	capKey2 := sdk.NewKVStoreKey("key2")
	schemaOpt := SetSubstores(capKey, capKey2)
	app := NewBaseApp(name, logger, db, nil, schemaOpt)
...
func TestCheckTx(t *testing.T) {
	...
	routerOpt := func(bapp *BaseApp) {
		...
		bapp.Router().AddRoute(sdk.NewRoute(routeMsgCounter,
...
func TestABCIEndToEndFraudProof(t *testing.T) {
	...
	appHashAfterFraud, err := appB1.cms.(*multi.Store).GetAppHash()
	require.Nil(t, err)
	...
	generateResp := appB1.generateFraudProofWithRouterOpts(
		abci.RequestGenerateFraudProof{
			BeginBlockRequest: *beginRequest, DeliverTxRequests: append(nonFraudulentDeliverRequests, fraudDeliverRequest), EndBlockRequest: nil,
		},
		routerOpts,
